...
/**
 * Manual trigger for the two maintenance functions from 0011 — expiring
 * stale job postings and downgrading memberships past their ends_at. Meant
 * as a fallback for projects without pg_cron enabled; see README for the
 * scheduled alternative.
 */
export async function runMaintenanceTasks(): Promise<{ jobsExpired: number; membershipsDowngraded: number } | null> {
  const supabase = await assertAdmin();
  if (!supabase) return null;

  const [{ data: jobsExpired }, { data: membershipsDowngraded }] = await Promise.all([
