...
export function MaintenanceRunner() {
  ...
  function handleRun() {
    ...
    startTransition(async () => {
      ...
      if (res) {
        setResult(`Expired ${res.jobsExpired} stale job(s), downgraded ${res.membershipsDowngraded} lapsed membership(s).`);
      }
    });
  }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-[13px] text-mist">
            Expires stale job postings and downgrades lapsed memberships. Runs automatically if
            you've set up pg_cron (see README) — this is the manual fallback.
          </p>
        </div>
        <button
          onClick={handleRun}
