...
import { updatePipelineStage, recordPlacement } from "@/lib/supabase/recruiter-actions";
...
export function PlacementLogger({
  ...
  const [state, formAction] = useFormState(recordPlacement, initialState);
  ...
  return (
    <form action={formAction} className="mt-3 flex flex-col gap-2.5 rounded-card border border-line bg-plum-50/50 p-4">
      ...
      <div className="flex gap-2">
        <button
          type="submit"
          className="rounded-pill bg-plum-600 px-4 py-2 text-[13px] font-medium text-white hover:bg-plum-700"
        >
          Save placement
        </button>
        <button
